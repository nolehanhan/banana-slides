...
def _build_single_image_provider(model: str = "gemini-3-pro-image-preview") -> ImageProvider:
    """按当前配置构造**单个**图片 provider（原 get_image_provider 的实现）。

    [自定义 2026-09-20] 原函数改名为内部构造函数，由下方新的 get_image_provider
    包一层「多源轮询兜底」。这样 ai_service_manager 的缓存路径与
    AIService.__init__ 直接构造的路径都能拿到兜底能力，无需逐处改动。
    """
...
def get_image_provider(model: str = "gemini-3-pro-image-preview") -> ImageProvider:
    """Factory: return the image provider, optionally wrapped in a multi-source fallback.

    [自定义 2026-09-20] 在原单源实现之上增加「多源轮询 / 失败兜底」：

    - 配置了 ``IMAGE_FALLBACK_1_*`` 等备用源时，返回一个按顺序尝试多个源、
      失败自动换源并冷却的包装 provider（实现见 ``ai_providers/fallback.py``）；
    - **没配任何备用源时，返回值与改动前完全一致**（直接是原单源 provider）。

    这样包装对所有调用方透明：无论走 ai_service_manager 的缓存，还是在
    AIService.__init__ 里直接构造，都能拿到兜底能力。
    """
    provider = _build_single_image_provider(model)
    try:
        from .fallback import maybe_wrap_image_provider
        return maybe_wrap_image_provider(provider, model)
    except Exception as exc:  # pragma: no cover - 兜底层自身故障绝不能影响生图主流程
        logger.error("装配图片多源兜底失败，回退为单源运行：%s", exc, exc_info=True)
        return provider
