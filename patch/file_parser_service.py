...
class FileParserService:
    ...
    def __init__(self, mineru_token: str, mineru_api_base: str = "https://mineru.net",
        ...
        Args:
            ...
            ai_provider_format: [自定义 2026-09-20] ``provider_format`` 的别名。
                上游 ``controllers/page_controller.py`` 调本类时用的是这个名字，
                而本类只声明了 ``provider_format``，导致「重新解析此页」
                （翻新项目重新解析并生成该页大纲/描述）必然抛
                ``TypeError: got an unexpected keyword argument 'ai_provider_format'``。
                此处补上别名，两个名字都认。
            upload_folder: [自定义 2026-09-20] 上游 ``page_controller`` 还会传这个参数，
                但本类原实现既不声明也不使用它（MinerU 落盘路径是自行推导的）。
                这里显式接收并仅作保存，保持行为不变。
            ignored_kwargs: [自定义 2026-09-20] 兜底：任何未声明的关键字参数一律忽略并告警，
                避免上游再出现类似不一致时直接抛 TypeError 打断整个功能。
        """
        ...
        # [自定义 2026-09-20] 两个参数名都接受，谁给值用谁
        self._provider_format = _get_ai_provider_format(provider_format or ai_provider_format)
        self._caption_provider = None
        # [自定义 2026-09-20] 原实现不使用，仅接收以免 TypeError
        self.upload_folder = upload_folder
        if ignored_kwargs:
            logger.warning(
                "FileParserService 收到未识别的参数，已忽略: %s", sorted(ignored_kwargs)
            )
    ...
    def _parse_pdf_file(self, file_path: str, filename: str) -> Optional[tuple]:
        ...
        Returns the standard parse_file tuple, or None when the PDF has no
        usable text layer (e.g. a scanned document), so the caller can fall
        back to MinerU OCR.
        """
        try:
            import fitz  # PyMuPDF
        except ImportError:
            logger.warning("PyMuPDF unavailable, skipping local PDF extraction.")
            return None

        try:
            doc = fitz.open(file_path)
            try:
                # sort=True restores natural reading order for multi-column layouts
                pages_text = [t for t in (page.get_text(sort=True).strip() for page in doc) if t]
            finally:
                doc.close()
            ...
            logger.info(
                f"PDF parsed locally: {len(pages_text)} page(s) with text, "
                f"{len(content)} characters"
            )
            return None, content, None, None, 0

        except Exception as e:
            logger.warning(f"Local PDF extraction failed for {filename}: {e}")
            return None
